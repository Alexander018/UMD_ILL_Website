...
function ParseCitation(){
	...
	var match = Object.keys(regexes).some(function(regex_name) {
		...
		if (!(result==null)){
		if (result.doi) { 
		    ga("send", {hitType: "event", eventCategory:"doi_resolver_button", eventAction:"click", eventLabel:"DOI identified"});
			ResolveDOI(result.doi); 
			return true
		}
	...
	if (!match) {
		ga("send", {hitType: "event", eventCategory:"doi_resolver_button", eventAction:"click", eventLabel:"citation parsing failed"});
  ...
  var xmlhttp = new XMLHttpRequest();
    xmlhttp.onloadend = function() {
      if (xmlhttp.status == 200) {
		ga("send", {hitType: "event", eventCategory:"doi_resolver_button", eventAction:"click", eventLabel:"DOI resolved"});
        FillFormFromJSON(JSON.parse(this.responseText));
		checkOpenAccess(doi_url)
        return true;
      } else if (xmlhttp.status == 404) {
		ga("send", {hitType: "event", eventCategory:"doi_resolver_button", eventAction:"click", eventLabel:"DOI resolution failed"});
...
function FillFormFromCitation(result) {
	//console.log(result)	ga("send", {hitType: "event", eventCategory:"doi_resolver_button", eventAction:"click", eventLabel:"citation parsed"});
